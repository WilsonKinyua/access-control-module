...
export class ProjectsService {
  ...
  async create(createProjectDto: CreateProjectDto, req: any): Promise<Project> {
    try {
      ...
      if (createProjectDto.assignedUsers?.length) {
        const users = await this.userRepository.findByIds(createProjectDto.assignedUsers);
  ...
  async findAll(req: any): Promise<Project[]> {
    try {
      ...
      if (req.user.role.name === 'Admin') {
        return await this.projectRepository.find({ relations: ['assignedUsers'] });
      } else {
  ...
  async findOne(id: number, req: any): Promise<Project> {
    try {
      ...
      const project = await this.projectRepository.findOne({ where: { id }, relations: ['assignedUsers'] });
      ...
      const isUserAssigned = project.assignedUsers.some(user => user.id === req.user.id);
  ...
  async update(id: number, updateProjectDto: UpdateProjectDto, req: any): Promise<Project> {
    try {
      ...
      if (updateProjectDto.assignedUsers?.length) {
        const users = await this.userRepository.findByIds(updateProjectDto.assignedUsers);
