...
export class HttpRequestExceptionFilter implements ExceptionFilter {
        ...
        if (exception instanceof HttpException) {
            ...
            if (typeof response === 'object' && response !== null && 'message' in response) {
                return {
                    ...
                    details: Array.isArray((response as any).message) ? (response as any).message : [(response as any).message],
                };
            }
    ...
    private formatDatabaseError(exception: QueryFailedError): { message: string, details?: string[] } {
        const driverError = exception.driverError as any;

        if (driverError.code === 'ER_DUP_ENTRY') {
            // Handle MySQL duplicate entry error
            const match = /Duplicate entry '(.*?)' for key '(.*?)'/.exec(driverError.message);
            if (match) {
                const value = match[1];
                const field = match[2];
                return {
        ...
        return { message: 'Database error occurred', details: [driverError.message] };
    }
}
