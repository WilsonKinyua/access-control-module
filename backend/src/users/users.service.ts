...
export class UsersService {
  ...
  async findUserById(id: number): Promise<User> {
    try {
      const user = await this.userRepository.findOne({ where: { id }, relations: ['role'] }); // Get a user by ID with their role
      if (!user) {
  ...
  async findByEmail(email: string): Promise<User> {
    try {
      const user = await this.userRepository.findOne({ where: { email }, relations: ['role'] }); // Get a user by email with their role
      if (!user) {
